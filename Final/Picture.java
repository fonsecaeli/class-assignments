...
//conventions class name must be capitalized.  Also final is a keyword so thats also awk
//Version 1.0
...
public class Picture {  
   ...
   */
   public static Point LOCATION = new Point(260, 280);
   ...
   */
   public static void drawLevyC(String path, Graphics g) {
      ...
      int turnAngle = 45;
      int maxAngle = 360;
      ArrayList<Color> colours = color(path.length());   
   ...
   /**
   * creates a list of colors which will fade, but will tend twords red
   *
   * @param length the number of colors needed, based of the number of segments making up the 
   * Levy C curve 
   * @return a list containing a bunch of colors 
   */
   public static ArrayList<Color> color(int length) { 
      ArrayList<Color> colours = new ArrayList<Color>();
      int redOffset = 150; //to make the color list generated tend more twords red
      for (int b = 0; b < 255; b++) {
         for (int g = 0; g < 255; g++) {
            for (int r = redOffset; r < 255; r++) {
               colours.add(new Color(r,g,b));
               if(colours.size() == length) {
                  return colours;
               }
            }
         }
   ...
   */
   public static void drawSegment(int theta, Graphics g) {
      ...
      switch(theta) { //switch statement seemed easiest here
         case 0:
            next = new Point(LOCATION.x, LOCATION.y+segmentLength);
            break;
         case 45: 
            next = new Point(LOCATION.x+segmentLength, LOCATION.y+segmentLength);
            break;
         case 90: 
            next = new Point(LOCATION.x+segmentLength, LOCATION.y);
            break;
         case 135: 
            next = new Point(LOCATION.x+segmentLength, LOCATION.y-segmentLength);
            break;
